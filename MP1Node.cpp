...
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message %s received sai kich thuoc.", label);
#endif
        return false;
    }
...
#ifdef DEBUGLOG
        log->LOG(&memberNode->addr, "Message JOINREP received with size . size[%i]", size);
#endif
        return false;
    }
